...
function IncomePage() {
  return <Layout><Meta title="Ingresos desde casa" description="Conoce de forma transparente la oportunidad independiente que Adrian presenta desde Smart Nutrition." /><main>
    <PageIntro eyebrow="Oportunidad independiente" title={<>Construye algo propio,<br /><em>con información real.</em></>} copy="La oportunidad se entiende mejor cuando se conversa sin adornos. Aquí tienes un primer contexto; el detalle llega en una charla personal." />
     <section className="mx-auto max-w-[1240px] px-5 pb-24 lg:px-8 lg:pb-32"><div className="glass overflow-hidden rounded-[2rem] p-3 sm:p-5"><div className="overflow-hidden rounded-[1.5rem] border border-[#d7a940]/20 bg-[#07101a]"><video className="aspect-video w-full object-cover" controls preload="metadata" playsInline poster={referenceImage} data-testid="video-income-opportunity"><source src={opportunityVideo} type="video/mp4" />Tu navegador no soporta la reproducción de video.</video></div><div className="px-3 pb-3 pt-6 sm:px-4"><Eyebrow>Video de introducción</Eyebrow><h2 className="max-w-2xl font-display text-3xl leading-tight text-[#f0ebdf] sm:text-4xl">Conoce la oportunidad desde una conversación directa.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-[#9ca8b1]">Reproduce el video cuando quieras. Después, si te quedan preguntas, Adrian puede orientarte personalmente.</p></div></div></section>
