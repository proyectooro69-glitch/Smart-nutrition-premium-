...
function GoldButton({ href, children, testId, secondary = false, message }: { href?: string; children: ReactNode; testId: string; secondary?: boolean; message?: string }) {
      ...
      <main>
        ...
        <section className="mx-auto max-w-[1240px] px-5 py-24 lg:px-8 lg:py-32">
          ...
          <div className="mt-12 grid gap-5 lg:grid-cols-3">
            <PathwayCard number="01" icon={<HeartHandshake size={22} />} title="Salud y bienestar" text="Conoce el espacio de productos y orientación de Smart Nutrition. El catálogo se habilita conforme hay información verificada para compartir." href="/salud-y-bienestar" testId="card-pathway-salud" />
...
function PathwayCard({ number, icon, title, text, href, testId }: { number: string; icon: ReactNode; title: string; text: string; href: string; testId: string }) {
  return <Link href={href} data-testid={testId} className="group glass relative flex min-h-[265px] flex-col justify-between overflow-hidden rounded-[1.5rem] p-7 transition-transform duration-500 hover:-translate-y-1"><div className="flex items-start justify-between"><span className="font-caps text-[10px] text-[#d7a940]">{number}</span><span className="text-[#e5b84f]">{icon}</span></div><div><h3 className="font-display text-3xl text-[#f0ebdf]">{title}</h3><p className="mt-3 max-w-md text-sm leading-6 text-[#929da7]">{text}</p><span className="mt-5 flex items-center gap-2 text-xs font-bold uppercase tracking-[.12em] text-[#e5b84f]">Conocer más <ArrowRightIcon /></span></div></Link>;
}
...
function WellnessPage() {
  return <Layout><Meta title="Salud y bienestar" description="Consulta el espacio de productos y orientación de Smart Nutrition. El catálogo se habilitará con información verificada." /><main>
    ...
    <section className="mx-auto max-w-[1240px] px-5 pb-24 lg:px-8 lg:pb-32"><div className="glass relative overflow-hidden rounded-[2rem] p-8 sm:p-12 lg:p-16"><div className="absolute -right-16 -top-24 h-72 w-72 rounded-full border border-[#d7a940]/20" /><div className="absolute -right-2 -top-10 h-48 w-48 rounded-full border border-[#d7a940]/10" /><div className="relative max-w-2xl"><div className="flex h-12 w-12 items-center justify-center rounded-full border border-[#d7a940]/45 text-[#d7a940]"><Leaf size={21} /></div><p className="mt-8 font-caps text-[10px] font-bold text-[#d7a940]">Próximamente</p><h2 className="mt-3 font-display text-4xl text-[#f0ebdf] sm:text-5xl">Nada de productos<br /><em className="text-[#d7a940]">puestos al azar.</em></h2><p className="mt-5 max-w-lg text-sm leading-6 text-[#9ca8b1]">Cuando el catálogo esté listo, encontrarás aquí una selección clara para conversar sobre tus objetivos y tus dudas. Mientras tanto, puedo orientarte personalmente.</p><div className="mt-8"><GoldButton message="Hola Adrian, quiero hablar sobre salud y bienestar." testId="link-wellness-whatsapp">Preguntar por WhatsApp</GoldButton></div></div></div></section>
